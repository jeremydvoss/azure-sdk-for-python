...
# TODO: JEREVOSS: Remove?
_DEFAULT_SPAN_ID = 0
_DEFAULT_TRACE_ID = 0

__all__ = ["AzureMonitorEventExporter"]
...
class AzureMonitorEventExporter(BaseExporter, LogExporter):
    """Azure Monitor Log exporter for OpenTelemetry."""

    def export(
        #TODO: JEREVOSS: not Log Data
        self, batch: Sequence[Event], **kwargs: Any  # pylint: disable=unused-argument
    ) -> LogExportResult:
        """Export log data.
...
def _get_log_export_result(result: ExportResult) -> EventExportResult:
    if result == EventExportResult.SUCCESS:
        return EventExportResult.SUCCESS
    if result in (
        ExportResult.FAILED_RETRYABLE,
        ExportResult.FAILED_NOT_RETRYABLE,
    ):
        return EventExportResult.FAILURE
    return None
